...
def _login_rate_limit():
    ...
    used to swallow the error and pass the request through (no-op bug).
    Limits are now hardcoded strings here and overridable via LOGIN_RATE_LIMIT
    env var read once at startup (see below).
    """

# Read limit strings once at import time from environment, with safe fallbacks.
# These never change at runtime, so importing them at module level is correct.
import os as _os
